...
def mapSynconsLang(tsv,lang):
  ...
  print 'Hay ' + str(totalsyncons) + ' en total'
  print 'No se encontraron ' + str(nf) + ' syncons en ' + lang
  print 'Se encontraron ', len(it_lang)
  return result
...
def paralellizeVectors(source,target,slang):
  ...
  for s_token in source[0]:
    if '#' in s_token and translation_s[s_token] in target_embeddings:
      target_values.append(target_embeddings[translation_s[s_token]])
      target_tokens.append(translation_s[s_token])
    else:
      if translation_l[s_token] in target_embeddings:
        target_values.append(target_embeddings[translation_l[s_token]])
        target_tokens.append(translation_l[s_token])
...
def generateBin(embeddings,path):  
  ...
  with open(path+'/vecs.bin','w') as binf:
...
def main():
  '''
  en_rows = os.getcwd()+'/word2vec/new_en-es_en/row_embedding.tsv'
  es_rows = os.getcwd()+'/word2vec/new_en-es_es/row_embedding.tsv'
  ...
  print 'Generando embeddings en ingles'
  start = time.time()
  en_cols = os.getcwd()+'/word2vec/new_en-es_en/col_embedding.tsv'
  en = createFullEmbeddings(en_rows, en_cols, 'en')#,'sl') # Tamano maximo 147456
  print 'Fin:', time.time() - start
  print 'Generando embeddings en espanol'
  start = time.time()
  es_cols = os.getcwd()+'/word2vec/new_en-es_es/col_embedding.tsv'
  es = createFullEmbeddings(es_rows, es_cols, 'es')#,'sl') # Tamano maximo 143360
  print 'Fin:', time.time() - start
  '''
  ...
  #  for x in es[1]:
  #    txt.write(str(x)+'\n')
  #generateBin(en,os.getcwd()+'/word2vec/new_en-es_en/bin-FULL')
  #generateBin(es,os.getcwd()+'/word2vec/new_en-es_es/bin-FULL')
