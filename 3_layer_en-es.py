...
class Sgd:
    ...
    global num_steps, batch_size, emb_size, h1_size, h2_size
    ...
    def train_graph(self):
        ...
        with graph.as_default():
            ...
            # Second layer
            w_shape2 = [h1_size,h2_size]
            weights2 = tf.get_variable("weights2", w_shape2, initializer=tf.contrib.layers.xavier_initializer())
            biases2 = tf.Variable(tf.zeros([h2_size]))

	    # Third layer
            w_shape3 = [h2_size,emb_size]
            weights3 = tf.get_variable("weights3", w_shape3, initializer=tf.contrib.layers.xavier_initializer())
            biases3 = tf.Variable(tf.zeros([emb_size]))
            ...
            loss = batch_loss + beta*tf.nn.l2_loss(weights1) + beta*tf.nn.l2_loss(weights2) + beta*tf.nn.l2_loss(weights3)
    ...
    def test_graph(self, ttype):
        ...
        with tf.Session() as session:
            ...
            with codecs.open(os.getcwd()+'/samples_'+self.svlang+'_500.txt','r',encoding='utf-8') as s:
                for line in s:
                    	samples[line.strip('\n')] = aux[line.strip('\n')]
	    ...
	    if ttype == 'distance':
                metrics.distances(embeddings, samples, self.sv, self.tv, 'NN3')
		#metrics.distance_from_algorithm(samples, self.sv, self.tv, 'NN3')
            else:
		start = time.time()
                metrics.top5(t,embeddings, samples, self.sv, self.tv, 'NN3')
                #metrics.top5(t, embeddings, samples, self.sv, self.tv, 'NN3')
		print('Fin:',time.time()-start)
